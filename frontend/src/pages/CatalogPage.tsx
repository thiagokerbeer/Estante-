...
function CoverFallback() {
  return (
    <div className="absolute inset-0 flex items-center justify-center bg-stone-800">
      <svg className="h-10 w-10 text-stone-500" fill="none" viewBox="0 0 24 24" stroke="currentColor" aria-hidden>
...
function BookCover({ src, title }: { src: string | null; title: string }) {
  ...
  if (!src || broken) return <CoverFallback />;
  return (
    <img
      src={src}
      alt={title}
        ...
        <ul className="mt-8 grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
          {filtered.map((b) => (
            <li key={b.id}>
              ...
              >
                <div className="relative aspect-[3/4] overflow-hidden bg-stone-800" aria-hidden>
